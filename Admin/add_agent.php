...
include('../connection.php');
		// include('conn.php');
	$client_last_id = '';
	$query = "select * from agent_data order by agent_id desc limit 1";

	$stmt = $connection->query($query);
    if(mysqli_num_rows($stmt) > 0) {
        if ($row = mysqli_fetch_assoc($stmt)) {
            $agent_last_id = $row['agent_id'];
            $agent_last_id = substr($agent_last_id, 3, 7);//separating numeric part
            $agent_last_id = $agent_last_id + 1;//Incrementing numeric part
            $agent_last_id = "CL-" . sprintf('%04s', $agent_last_id);//concatenating incremented value
            $agent_id = $agent_last_id;
        }
    } 
    else {
        $agent_last_id = "CL-0001";
        $agent_id = $agent_last_id;
    }
                                ...
                                <div class="row">
									<div class="col-md-6 col-sm-12">
										<div class="form-group">
											...
											<input type="text" name="agent_name" class="form-control" placeholder="ex. Evans" requried>
										</div>
									</div>
                                ...
                                <div class="row">
                                    <div class="col-md-6 col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <input type="email" name="agent_email" class="form-control" placeholder="example@example.com" required>
                                        </div>
                                    </div>
                                    <div class="col-md-6 col-sm-12">
                                        <div class="form-group">
                                            <label>Password</label>
                                            <input type="password" id="password" name="agent_password" class="form-control" placeholder="********" required>
